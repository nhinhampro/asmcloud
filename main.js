...
hbs.registerHelper("priceColor", function (price) {
    if (price >= 500 && price < 1000) return 'red'
    else if (price >= 1000) return 'blue'
    else return 'green'
})
hbs.registerHelper("quantityColor", function (quantity) {
    if (quantity < 10) return 'red'
    else if(quantity < 50) return 'green'
    else return 'blue'
})
...
app.post('/new',async (req,res)=>{
    ...
    const category = req.body.txtCategory
    if(name.length < 5 ){
        res.render('newProduct', {'warning': "Fields name must have more 5 keywords"})
    }
    else if(price <10){
        res.render('newProduct', {'warning': "Price must be more than 10"})
    }
    else if(quantity.length == 0 || price.length == 0 || picUrl.length == 0 || category.length == 0){
        res.render('newProduct', {'warning': "Please fill out name fields"})
    }
    else{
        const newProduct = {
            name :name,
